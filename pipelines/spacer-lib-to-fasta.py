...
c = db.cursor()
c.execute("""SELECT ce.id,ce.spacer,ca.id,cs.refseq_id,cs.organism_name FROM crispr_crisprentry ce JOIN crispr_crisprarray ca ON ce.array_id = ca.refseq_id_id JOIN crispr_strain cs ON ca.refseq_id_id = cs.id""")
